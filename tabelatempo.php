...
<body>
    <div class="container">
                ...
                <!-- WIDGET:CATEGORIES - START -->
                    <div class="widget widget-navigation">
                        <ul class="nav nav-pills nav-stacked">
                            ...
                            <li class="active"><a href="tabelatempo.php">Tabela de tempo</a></li>
        ...
        <select class="form-control" name="diff" id="diff" onchange="dificuldade()" require>
            <option value="temp_facil">Fácil</option>
            <option value="temp_medio">Médio</option>
            <option value="temp_dificil">Difícil</option>
            <option value="temp_mestre">Mestre</option>
        </select>
        </label>
...
=========================== -->        
    ...
    <script type="text/javascript">
        var dific="";
        var query="SELECT * FROM `tempo` ";
        var query2=" ORDER BY `"+dific+"` ASC ";
        function dificuldade(){
            dific=document.getElementById('diff').value;
            query2=" ORDER BY `"+dific+"` ASC ";
            pesquisa();
        }
        dificuldade();
    </script>
    ...
    function pesquisa(){
        ...
        if (string=='()'){
            var sql=query+ " WHERE `temp_facil`='bomdia'";
            var sql_limit=sql+ " LIMIT "+user_count;
        } else {
            var sql=query+ " WHERE `id_jogador` IN " + string + query2 ;
            var sql_limit=sql+ " LIMIT "+user_count;
        }

        $.ajax({
            url:'/ajax_tabelatempo.php',
            method:'POST',
            data:{sql:sql_limit,dific:dific},
            error:function(){
